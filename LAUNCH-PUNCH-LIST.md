...
- [x] **C7. Checked-in `AndroidManifest.xml` predates `app.json`** — **RESOLVED 2026-07-28.** `android:usesCleartextTraffic="true"` now on the `<application>` element. **This entry's diagnosis was wrong in a way that mattered:** it assumed `app.json` was already correct and only the native manifest had drifted. In fact `android.usesCleartextTraffic` is **not in the Expo SDK 56 config schema and no config-plugin mod reads it** — verified by `expo prebuild --clean` in a scratchpad copy producing a manifest with no such attribute. The app.json key was inert, so a prebuild would NOT have fixed this. Fixed with a real config plugin (`GroceryApp/plugins/withCleartextTraffic.js`) **plus** the hand-patched manifest, so generation reproduces it. `android/app/src/main/AndroidManifest.xml`
...
- [x] ~~Sentry triple-version-mismatched, DSN never inlined — crash reporting dead in production.~~ **FIXED 2026-07-28.** Dropped deprecated `sentry-expo@7.2.0` (merged into `@sentry/react-native` as of SDK 50) — package, `app.json` `plugins[0]`, and the `src/services/sentry.ts` import all moved together. `npm ls @sentry/react-native` is now a single `8.14.0` (was `8.14.0` + nested `5.17.0`). `SENTRY_DSN` → `EXPO_PUBLIC_SENTRY_DSN`; without the prefix the Expo bundler never inlines it. **Proven by sentinel export:** `sentinel0000` present in the emitted `.hbc`; negative control with the unprefixed name absent. `src/services/sentry.ts`
...
- [x] ~~**Two empty-states tell the user to do something that is now impossible**~~ *(found 2026-07-28, fallout of `a9792cdd`)* — **FIXED 2026-07-28 at the owner's direction.** Both Deals surfaces rendered *"Please connect a Turso database in settings…"* long after Stage 1 deliberately removed the URL and token fields, so the instruction could not be followed and it named a third-party vendor to end users. New copy names the control that actually exists — *"Please turn on Product Catalog Lookups in settings… If it is already on, no relay is configured yet"* — which matches the `SettingsScreen` label verbatim and covers **both** arms of the gate (`isCatalogAvailable()` is false when the toggle is off **or** no relay is set). Internal key `turso_missing` → `catalog_unavailable` (4 sites, no test coupling). Guarded by a new case in `__tests__/no-client-db-credential.test.ts`, **proven to fail** by replanting the old string. `src/screens/HomeScreen.tsx:663`, `src/screens/GroceryListScreen.tsx:796`
...
- [x] **Release build falls back to the committed debug keystore when no upload keystore is set** — **VERIFIED CORRECT-BY-DESIGN 2026-07-28, not a defect to fix.** The `signingConfigs.release` block reads `PANTRYRUN_UPLOAD_*` behind `project.hasProperty`, and the fallback exists only so a local `assembleRelease` works without an upload key. Proven end-to-end with a disposable scratchpad keystore: passing `-PPANTRYRUN_UPLOAD_STORE_FILE=` produces an artifact signed by the supplied cert, **not** `CN=Android Debug`. Play Console rejects debug-signed bundles, so this cannot ship by accident. `android/app/build.gradle`
- [x] ~~`autoVerify` intent filter mixes custom scheme with https — App Links verification may fail.~~ **FIXED 2026-07-28.** The `groceryapp://` scheme moved to its own filter; only `https` remains under `autoVerify`. The hand-patched result is **byte-identical** to what `expo prebuild` generates from `app.json` (which already declared them as two separate filters), so the fix matches continuous native generation rather than diverging from it. `android/app/src/main/AndroidManifest.xml`
...
- [ ] **Invite-redirect points Android users at a nonexistent Play listing** *(found 2026-07-28, goal-4 pass — recorded, deliberately NOT fixed there: it is web content, not build configuration)* — `androidStoreUrl` is `https://play.google.com/store/apps/details?id=com.groceryapp.app`, the **wrong package**. Correct value: `com.shiftlogichq.pantryrun`. Every Android user who taps an invite link without the app installed lands on a dead listing. The adjacent `iosStoreUrl` also still carries the placeholder `idXXXXXXXXXX` and needs the real App Store ID once the record exists. **Note the path** — the goal prompt cites `relay-server/public/…`, but the only copy in the repo is under `GroceryApp/`. `GroceryApp/relay-server/public/invite-redirect.html:112-113`
- [ ] **`*.keystore` is not git-ignored** *(found 2026-07-28)* — `GroceryApp/.gitignore` covers `*.jks` but not `*.keystore`, so a real upload keystore created inside the working copy would be stageable and committable. (`android/app/debug.keystore` is tracked deliberately — stock RN debug key, credentials public by design — which is why the pattern was not simply added.) Owner guidance: keep the upload keystore outside the repo entirely. `GroceryApp/.gitignore`
- [ ] **Stale token-bearing bundle still on local disk** *(found 2026-07-28)* — `GroceryApp/android/app/src/main/assets/index.android.bundle` and `GroceryApp/dist-android/` are untracked and gitignored (the purge landed), but both still exist **on this machine** and the bundle still contains 1 JWT / 2 `turso.io` strings. Not a repo defect and not reachable by git; local cleanup only. `GroceryApp/android/app/src/main/assets/index.android.bundle`
...
- [ ] **Two lockfiles are tracked for one package manager** *(found 2026-07-28)* — `GroceryApp/` carries both `package-lock.json` and `yarn.lock`. CI, EAS and the Dockerfile all use `npm ci`, so `yarn.lock` is vestigial — but `npm ci` rewrites it as a side effect, so it shows up as a spurious ~1,400-line diff after any install and invites someone to commit churn or, worse, to trust it. **RESOLVED 2026-07-28:** `package-lock.json` is authoritative (CI, EAS and `relay-server/Dockerfile` all use `npm ci`); `yarn.lock` was `git rm`'d, and `"packageManager": "npm@10.9.8"` added to `GroceryApp/package.json` so the manager is pinned rather than merely conventional. `GroceryApp/package.json`
