...
- [ ] **`npm ci` fails in `GroceryApp/` — 58 mismatched packages** *(found 2026-07-28, scope: G4)* — `expo-image-picker@~56.0.20` is in `package.json` and **missing from the lockfile**, and the whole Expo set is skewed (`expo` 56.0.8 vs 56.0.17, `expo-modules-core` 56.0.14 vs 56.0.22, `expo-constants`, `expo-linking`, `expo-image-manipulator`, `lightningcss-*`, `nanoid`). That is the fingerprint of the iOS session bumping Expo versions without regenerating the lockfile. **CI and EAS both run `npm ci`, so every PR is red until this is regenerated** — and a permanently red baseline is how a real failure gets ignored. One-command fix; deliberately not done on the security branch. `GroceryApp/package-lock.json`
