...
### A. Trip Optimizer gated off
- `TRIP_OPTIMIZER_ENABLED = false`, module-level in `src/screens/GroceryListScreen.tsx` with a doc comment (why off, what re-enabling requires), guarding the single `<StopOptimizer` JSX site with the direct `{FLAG && (…)}` idiom copied from `MANAGED_TIER_ENABLED`. `<TripPlanSheet` renders only inside `StopOptimizer.tsx`, so it's transitively unreachable; no second guard added there (the const is module-local by design). Nothing deleted — implementation, pricing modules and `stop-optimizer.test.ts` intact for 1.x flag-flip + IAP entitlement.
- Purchase-copy sweep (premium|upgrade|subscribe|subscription|paywall|unlock, case-insensitive over `src/` + `App.tsx`): 28 hits, all false positives (BIP39 words, price-normalizer stopwords, RN "subscribe/subscription" senses, schema "upgrade", decrypt-sense "unlock") or deliberately-kept managed-tier plumbing. The literal "Managed Subscription" / "Subscription Key" / "Enter your subscription key" strings in SettingsScreen stay: they sit inside `{!isSelfHosted && (` where `isSelfHosted = !MANAGED_TIER_ENABLED || …` is unconditionally true, so the block cannot render.
- New guard test `__tests__/v1-free-posture.test.ts`: reads the REAL files (note: `ac15-no-analytics.test.ts`, cited as the idiom's precedent, actually only checks inline mock arrays — the new test improves on it), proves its matcher rejects an unguarded fixture before applying it, and pins the flags, the guard, and TripPlanSheet's single parent. `ac6-tier-switch` untouched and green (no change to the `isSelfHosted` derivation).

### C. First-run recovery backup (launch-blocker)
- New settings flag `recoveryPhraseBackedUp` (types/index.ts, DEFAULT_SETTINGS `false`, KNOWN_SETTINGS_KEYS). **Only** RecoveryScreen's explicit "I've Stored It Safely" handler writes it to `true` — dismiss/back/backgrounding leave it unset, so the prompt re-surfaces next launch. This deliberately does NOT copy the `contributeConsentShown` bug (whose cancel/skip handlers set the flag).
- Trigger: HomeScreen effect — if flag unset and `hasRecoveryPhrase()`, navigate to `Recovery { mode: 'show' }` once per app launch. Reuses RecoveryScreen; no second phrase-rendering site.
- Copy updated for honesty: no account/email/reset, lose-all-devices+phrase = data unrecoverable; explicit note that joining a different family later replaces the phrase.
- **`recoverFromPhrase()` fix landed**: it now stores the seed, normalized phrase, and stored-flag under the familyId on success (best-effort, non-fatal), so joined-not-founded devices no longer hit `generateRecoveryPhrase()`'s hasMasterKey throw from Settings → View Recovery Phrase. `ac13-key-recovery` + `join-flow` stayed green (39/39). RecoveryScreen 'show' mode's null-phrase fallback now throws an actionable message ("enter your family's phrase in Recover from Backup") instead of the unexplained guard error when a master key exists.
- New test `__tests__/first-run-recovery-backup.test.ts`: default false, exactly one write-site (source scan), HomeScreen trigger present, and functional proof that recovery re-stores the phrase.

### D. Persistence errors surfaced
- Both `persistListToDB(...).catch` sites in `src/sync/sync-manager.ts` (Yjs update observer + remote-update path) now route through a shared `reportPersistFailure()`: keeps the console diagnostic and the handled rejection, and sets `useSyncStore` `{syncState:'error', error:"Couldn't save changes on this device"}` via dynamic import (static import would cycle — useSyncStore imports syncManager).
- Copy choice: "save … on this device", not "Sync error" — the failure is the local write; the network may be fine, and a wrong label sends users debugging the wrong thing. SyncIndicator now renders the store's `error` message (falls back to "Sync error") in the error state; message kept short for the one-line header row.
- New test `__tests__/persist-failure-surfacing.test.ts`: both paths functionally (mock-rejected persist → store error state) plus a source scan asserting no bare console.warn catch remains.

### Handoffs / deferred
- **Store-listings conflict (owner / G6):** `audit-package/07-STORE-LISTINGS.md` still advertises Trip Optimizer at (current) lines **96** (listing bullet), **160** (screenshot 4: Trip Plan sheet), **208** (video 18–24s beat). v1 has no Trip Optimizer; listings need rework around the private/shared/offline-first family list. File deliberately not edited here.
- **Device proof — DONE on Android emulator (iOS still blocked).** The first-run flow was verified end-to-end on a booted `Pixel_8_Pro` AVD running the debug APK built from this branch (`./gradlew assembleDebug`, BUILD SUCCESSFUL, 481 tasks). Verified behaviours, each by screenshot:
  1. **Fresh install → the Recovery Phrase screen appears on its own** after Home mounts (`pm clear` first, so genuinely first-run). New honest copy renders: no account/email/password reset, lose-all-devices-plus-phrase = unrecoverable, plus the "joining another family replaces this phrase" note.
  2. **Dismiss with Back → relaunch → the prompt comes back.** This is the acknowledgement semantics working: dismissing does not set `recoveryPhraseBackedUp`.
  3. **Tap "✓ I've Stored It Safely" → confirmation alert → relaunch → prompt gone**, Home shown. Only the explicit acknowledgement silences it.
  4. **Trip Optimizer gate holds at runtime**: the list screen renders store cards → category pills → "Find Prices" with no optimizer block in between, which is exactly where `<StopOptimizer>` used to sit.
  Redacted screenshot (phrase blacked out) kept OUTSIDE the repo at the session scratchpad as `first-run-recovery-REDACTED.png`; every unredacted capture was deleted and the emulator's app data wiped, so the generated phrase no longer exists anywhere.
- **iOS device proof still blocked (toolchain, not code):** `pod install` fails twice — (1) stock invocation crashes CocoaPods 1.17.0 with `Unicode Normalization not appropriate for ASCII-8BIT` (fix: UTF-8 locale), then (2) with `LANG=en_US.UTF-8` the RN 0.85 Podfile **refuses Rosetta**: every Ruby on this machine (Homebrew `/usr/local` Cellar and all rbenv versions) is x86_64, so `pod` runs emulated on this arm64 Mac and aborts before installing anything (`Pods/Target Support Files/` empty → xcodebuild fails on the missing `Pods-PantryRun.debug.xcconfig`). Shell is arm64; only the Ruby toolchain is Intel. Owner command from a native terminal with arm64 CocoaPods: `cd GroceryApp/ios && LANG=en_US.UTF-8 pod install && cd .. && npx expo run:ios --device "iPhone 17"`.

### NEW FINDING — a fresh `npm install` produced an app that cannot bundle (FIXED, owner-requested follow-up 2026-07-28)

**Fix landed:** `expo-asset@~56.0.21` and `expo-font@~56.0.7` added as direct dependencies (the exact versions `expo@56` already resolved), lockfile regenerated. The hoisted peer `expo-font@56.0.5` is gone; both packages now sit at top level where Metro resolves them. Proof: `npx expo export --platform android` produces a complete release Hermes bundle (7.1MB .hbc) where it previously failed with `UnableToResolveError`; tsc exit 0 and jest 47/514 unchanged. Original diagnosis below, kept for the record.

Original finding:
`App.tsx` does `await import('expo-font')`, and Metro fails to resolve it: **`Unable to resolve module expo-asset from node_modules/expo-font/build/FontLoader.js`**, error 500 from the dev server, red screen instead of the app. Cause: npm hoists a *peer* copy of `expo-font@56.0.5` to top-level `node_modules/expo-font` (it is not in `package.json` at all — it arrives as a peer of `@expo/vector-icons`), but `expo-asset` exists only nested at `node_modules/expo/node_modules/expo-asset`, alongside the `expo-font@56.0.7` that `expo` itself depends on. The hoisted 56.0.5 copy therefore has no resolvable `expo-asset`. Since `Font.loadAsync` sits inside `App.tsx`'s init `try`, the production failure mode is the "PantryRun couldn't start" error screen, and the same resolver runs for EAS/release builds — so this is a launch-blocking risk, not a dev-only annoyance. **Worked around locally for the device test with a symlink in gitignored `node_modules` (nothing committed, no dependency added — the goal's constraints forbid that).** Owner fix to evaluate: add `expo-asset` (and probably an explicit `expo-font`) to `package.json` at the versions `expo@56` already resolves, then re-lock and rebuild. Whoever owns dependencies/build should confirm whether the previously-successful iOS build at `43756d4` used a node_modules tree that happened to hoist differently.
- Prompt cadence decision: first-run prompt fires once per Home mount per app launch (not on every focus) — annoying-loop avoidance; unset flag guarantees it returns next launch.
