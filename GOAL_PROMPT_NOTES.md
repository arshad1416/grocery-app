...
- **Device proof of first-run flow — attempted, blocked by toolchain:** Expo CLI 56.1.21 resolves; iPhone 17 simulator booted (`C36FB7D9-…`); `ios/` project exists. `pod install` fails twice: (1) stock invocation crashes CocoaPods 1.17.0 with `Unicode Normalization not appropriate for ASCII-8BIT` (fix: UTF-8 locale), then (2) with `LANG=en_US.UTF-8` the RN 0.85 Podfile **refuses Rosetta**: every Ruby on this machine (Homebrew `/usr/local` Cellar and all rbenv versions) is x86_64, so `pod` runs emulated on this arm64 Mac and aborts before installing anything (`Pods/Target Support Files/` left empty → xcodebuild fails on missing `Pods-PantryRun.debug.xcconfig`). The shell itself is arm64; only the Ruby toolchain is Intel. Owner command, from a native terminal with an arm64 CocoaPods (e.g. arm64 Homebrew `brew install cocoapods`, or the setup used for commit 43756d4's successful iOS build): `cd GroceryApp/ios && LANG=en_US.UTF-8 pod install && cd .. && npx expo run:ios --device "iPhone 17"` — then fresh-install the app and the recovery-backup screen appears after Home mounts. The flow is proven statically + by test regardless; screenshot must use a synthetic/redacted phrase and stay out of the repo.
