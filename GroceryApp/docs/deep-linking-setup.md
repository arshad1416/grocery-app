...
2. The bundle identifier is settled: `com.shiftlogichq.pantryrun`. It must match `ios.bundleIdentifier` in `app.json` — do not substitute another value.
...
1. The Android package is settled: `com.shiftlogichq.pantryrun`. It must match `android.package` in `app.json` and `applicationId` in `android/app/build.gradle` — do not substitute another value.
